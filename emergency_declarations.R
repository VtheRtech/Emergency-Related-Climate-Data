...
res <- res %>%
  rename(
    Number_of_Disasters_Declared = number_of_incidents,
    Year_Declared = fy_declared
  )
...
# filter out outliers
# outliers filters where the year 2020(for covid-19),
# and 2023(because it was incomplete collection of information)
res <- res %>%
...
# build a standard GGPLOT since this graph
# is on a timelines no need to do anything to it
res.graph <- ggplot(data = res,
                    mapping = aes(x = Year_Declared,
                                  y = Number_of_Disasters_Declared)) +
  geom_point(aes()) +
  geom_smooth(method = "lm", col = "black") +
  theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust = 1)) +
  labs(
    title = "Number of Natural Disasters Declared by year", subtitle =
...
res_growth <- res %>%
  ...
  mutate(
    Diff_Year = Year_Declared - lag(Year_Declared),
...
Average_Growth <- mean(res_growth$Rate_Percent,
                       na.rm = TRUE)
